...
| `fit_mode`         | `'contain'                       | 'width'      | 'none'`                                                                                                                                                                                        | `'contain'`                                                                                                        | スケールの自動計算ルール。`'contain'`: 枠内に1ページが完全に収まるよう自動縮小。`'width'`: 横幅をコンテナの幅にぴったり合わせる。`'none'`: 自動計算を無効化し、`scale` の値を直接使用。 |
| `currentPageIndex` | `number`                         | `0`          | **双方向バインド**。表示中のスライドインデックス。`mode:'scroll'` では現在のスクロール位置に応じたインデックスが自動同期され、外部からのインデックス変更時は該当ページへ自動スクロールします。 |
| `scale`            | `number`                         | `1.0`        | **双方向バインド**。スライドのスケール倍率。`fit_mode` が `none` 以外の時は、自動計算された最新の倍率がこの変数に同期されます。                                                                |
...
| `scrollTop`        | `number`                         | `0`          | **双方向バインド**。親コンテナの `scrollTop`（スケール変形適用後の実際の物理ピクセル値）。                                                                                                     |
| `scrollLeft`       | `number`                         | `0`          | **双方向バインド**。親コンテナの `scrollLeft`（スケール変形適用後の実際の物理ピクセル値）。                                                                                                    |
| `scrollbarMode`    | `'always'                        | 'hidden'     | 'auto'`                                                                                                                                                                                        | `'auto'`                                                                                                           | `mode:'scroll'` 時の縦スクロールバー制御。`'always'`: 常に表示し有効領域から幅を予め減算。`'hidden'`: 表示を消す（ホイール等の操作は維持）。`'auto'`: はみ出た時のみ自動表示。          |
...
- **ランタイム環境のスクロールバー計測:** `onMount` ライフサイクル時に、実行環境（OS/ブラウザ）固有のスクロールバーの物理的な幅をダミー要素から正確に自動実測します（Macのスクロールバー自動非表示環境などの幅0pxにも完全適応）。
