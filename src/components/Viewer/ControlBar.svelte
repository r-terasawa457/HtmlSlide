<script lang="ts">
  ...
  function handlePageChange(e: Event) {
    ...
    if (!isNaN(val) && val >= 1 && val <= viewerState.totalPages) {
      viewerState.currentPage = val;
  ...
  async function handleExportPptx() {
    const title = viewerState.title || "presentation";
  ...
  async function handleOpenPresenter() {
    ...
    const width = 1280;
    const height = 720;
    viewerState.presenterWindow = window.open(
      presenterUrl,
      "presWin",
...
<div id="toolbar">
  ...
  <div class="toolbar-section">
    ...
    <button onclick={onPrint} title="印刷">
      <svg viewBox="0 0 24 24">
        <path
