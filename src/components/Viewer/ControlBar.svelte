<script lang="ts">
  ...
  async function handleOpenStageView() {
    const stageUrl = await AssetProvider.resolveCompositeHtmlUrl(
      "/src/entrypoint/stage_view.html",
    );
    const width = 1280;
    const height = 720;
...
<div class={viewerState.isMainFullscreen ? "fullscreen-toolbar-container" : ""}>
  <div
    id="toolbar"
    class="h-12 min-h-12 bg-[#323639] color-white flex items-center justify-between px-4 shadow-[0_2px_5px_rgba(0,0,0,0.3)] relative z-50 select-none"
  >
    ...
    <div class="flex items-center gap-2">
      <div class="flex items-center gap-1 text-sm text-white">
        <input
          ...
          min="1"
          max={viewerState.slideDataStore.slideMeta?.pageLength ?? 0}
        />
        <span class="text-gray-400"
          >/ {viewerState.slideDataStore.slideMeta?.pageLength ?? 0}</span
        >
      </div>
      ...
      <div class="flex items-center gap-1">
        <button
          ...
          title="縮小"
          class="p-1.5 rounded hover:bg-[rgba(255,255,255,0.1)] transition-colors"
        >
          <svg class="w-5 h-5 fill-white" viewBox="0 0 24 24"
            ><path d="M19 13H5v-2h14v2z" /></svg
          >
        </button>
        <span class="w-14 text-center text-sm text-white"
          >{viewerState.zoomPercentage}</span
        >
        <button
      ...
      <div class="flex items-center gap-0.5">
        <button
          ...
          title="等倍 (100%)"
          class="p-1.5 rounded transition-colors {viewerState.zoomMode ===
          'ORIGINAL'
            ? 'bg-[rgba(66,133,244,0.2)]'
            : 'hover:bg-[rgba(255,255,255,0.1)]'}"
        >
          <svg
            class="w-5 h-5 {viewerState.zoomMode === 'ORIGINAL'
              ? 'fill-[#4285f4]'
              : 'fill-white'}"
            viewBox="0 0 24 24"
            ><path
              d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zM12 7h-2v3H7v2h3v3h2v-3h3v-2h-3V7z"
            /></svg
          >
        </button>
        <button
        ...
        >
          <svg
            class="w-5 h-5 {viewerState.zoomMode === 'FIT_HEIGHT'
              ? 'fill-[#4285f4]'
              : 'fill-white'}"
            viewBox="0 0 24 24"
            ><path
              d="M13 2v20h-2V2h2zM5 10l2-2 2 2H5zm0 4l2 2 2-2H5zm14-4l-2-2-2 2h4zm0 4l-2 2-2-2h4z"
            /></svg
          >
        </button>
        <button
        ...
        >
          <svg
            class="w-5 h-5 {viewerState.zoomMode === 'FIT_WIDTH'
              ? 'fill-[#4285f4]'
              : 'fill-white'}"
            viewBox="0 0 24 24"
            ><path
              d="M2 13h20v-2H2v2zM10 5l-2 2 2 2V5zm4 0l2 2-2 2V5zm10 14l-2-2 2-2v4zm4 0l2-2-2-2v4z"
            /></svg
          >
        </button>
        ...
        <button
          ...
          title={viewerState.laserActive
            ? "レーザーポインターをオフ"
            : "レーザーポインターをオン"}
          class="p-1.5 rounded transition-colors ml-1 {viewerState.laserActive
            ? 'bg-[rgba(255,23,68,0.2)]'
            : 'hover:bg-[rgba(255,255,255,0.1)]'}"
        >
          <svg
            class="w-5 h-5 {viewerState.laserActive
              ? 'fill-[#ff1744]'
              : 'fill-white'}"
            viewBox="0 0 24 24"
          >
            ...
            <path
              ...
              stroke={viewerState.laserActive ? "#ff1744" : "#ffffff"}
              stroke-width="2"
    ...
    <div class="flex items-center gap-2">
      <button
        ...
        title={isExporting ? "PPTX生成中..." : "PPTXに出力"}
        class="p-1.5 rounded hover:bg-[rgba(255,255,255,0.1)] disabled:opacity-40 transition-colors"
      >
        <svg class="w-5 h-5 fill-white" viewBox="0 0 24 24"
          ><path
            d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H10v-4H7l5-5 5 5h-3v4z"
          /></svg
        >
      </button>
      <button
        onclick={handleOpenStageView}
        title="プロジェクター・別窓表示"
        class="p-1.5 rounded hover:bg-[rgba(255,255,255,0.1)] transition-colors"
      >
        <svg class="w-5 h-5 fill-white" viewBox="0 0 24 24"
      ...
      <button
        onclick={() => appState.requestPrint()}
        title="PDF印刷印刷（Ctrl+P）"
        class="p-1.5 rounded hover:bg-[rgba(255,255,255,0.1)] transition-colors"
      >
        <svg class="w-5 h-5 fill-white" viewBox="0 0 24 24"
          ><path
      ...
      <button
        onclick={() => viewerState.toggleMainFullscreen()}
        title="全画面表示"
        class="p-1.5 rounded hover:bg-[rgba(255,255,255,0.1)] transition-colors"
      >
