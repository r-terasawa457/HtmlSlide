...
import { SlidesEngine } from "./SlidesEngine.js";
...
function setupViewer(slidesHtml) {
    ...
    totalPagesLabel.textContent = document.querySelectorAll('.slide').length;
    ...
    function applyZoom() {
        ...
        ['Original', 'Custom', 'FitHeight', 'FitWidth'].forEach(m => {
            ...
            if (btn) btn.classList.toggle('active', currentMode === m.toUpperCase().replace('FIT', 'FIT_'));
        });
    }
    ...
    document.getElementById('btnOriginal').onclick = () => { currentMode = 'ORIGINAL'; updateLayout(); };
    document.getElementById('btnCustom').onclick = () => { currentMode = 'CUSTOM'; updateLayout(); };
    document.getElementById('btnFitHeight').onclick = () => { currentMode = 'FIT_HEIGHT'; updateLayout(); };
    document.getElementById('btnFitWidth').onclick = () => { currentMode = 'FIT_WIDTH'; updateLayout(); };
    document.getElementById('zoomIn').onclick = () => { currentMode = 'CUSTOM'; currentZoom += 0.1; applyZoom(); };
    document.getElementById('zoomOut').onclick = () => { currentMode = 'CUSTOM'; currentZoom = Math.max(0.1, currentZoom - 0.1); applyZoom(); };

    window.onresize = () => { if (currentMode.startsWith('FIT')) updateLayout(); };

    pageInput.onchange = (e) => {
        const target = document.getElementById('slide-' + e.target.value);
        if (target) viewer.scrollTo({ top: target.offsetTop * currentZoom, behavior: 'smooth' });
    };
    ...
    document.getElementById('presentBtn').onclick = () => {
        window.open('index.html?mode=present', 'presWin', `width=${BASE_WIDTH},height=${BASE_HEIGHT},menubar=no,toolbar=no,location=no,status=no`);
    };
